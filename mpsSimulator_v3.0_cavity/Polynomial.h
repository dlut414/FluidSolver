...
namespace SIM {
	...
	private:
		...
		struct Gen__			{ static __forceinline void Gen(const R* const x, R* const out) { out[I] = Power<PX>(x[0])*Power<P_-PX>(x[1]); Gen__<P_,PX-1,I+1>::Gen(x, out); } };
		template <unsigned P_, unsigned I>						
		struct Gen__<P_,0,I>	{ static __forceinline void Gen(const R* const x, R* const out) { out[I] = Power<P_>(x[1]); } };
		template <unsigned P_ = 1, unsigned I = 0, bool Over = !(P-P_)>
		struct Gen_				{ static __forceinline void Gen(const R* const x, R* const out) { Gen__<P_,P_,I>::Gen(x, out); Gen_<P_+1,I+H<2,P_>::value>::Gen(x, out); } };
		template <unsigned P_, unsigned I>
		struct Gen_<P_,I,true>	{ static __forceinline void Gen(const R* const x, R* const out) { Gen__<P_,P_,I>::Gen(x, out); } };
	public:
